...
class Rovnica:
    ...
    def __init__(self, koefA: float | None=None, koefB: float| None = None,*,generuj:bool = False ):
        ...
        """
        if not generuj:
            self.a = self.getNumber("Zadaj koeficient a:",-100,100,False) if koefA is None else koefA
            self.b = self.getNumber("Zadaj koeficient b:",-100,100,True) if koefB is None else koefB
        else:
